...
class LSTM(nn.Module):
    ...
    def __init__(self, input_dim, rstm_layers, hidden_dim, nn_specs):
        ...
        self.lstm = nn.LSTM(input_size= input_dim, num_layers=rstm_layers, hidden_size=hidden_dim,batch_first=True)
    ...
    def train(dataset, num_features, rstm_layers, hidden_dim, num_epochs, batch_size, nn_specs,lr=0.001, verbose="v"):

        data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
        model = LSTM(input_dim = num_features,rstm_layers= rstm_layers ,hidden_dim = hidden_dim, nn_specs=nn_specs)
